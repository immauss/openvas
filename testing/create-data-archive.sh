...
if ! [ -d /bind/$OLDTAG-bind ]; then
	mkdir /bind/$OLDTAG-bind
fi
docker run  -d --rm -e SKIPSYNC=true -v /bind/$OLDTAG-bind:/data --name $OLDTAG-bind -p 8081:9392 immauss/openvas:$OLDTAG
...
sudo tar cJvf /bind/$OLDTAG-bind.tar.xz /bind/$OLDTAG-bind
